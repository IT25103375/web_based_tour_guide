...
import { tourPackages, events as mockEvents } from "../data/mockData";
...
export default function EventBooking() {
  ...
  useEffect(() => {
    ...
    const fallback = mockEvents.filter((e) => e.packageId === selectedPackageId);
      ...
      if (Array.isArray(data) && data.length > 0) {
        setFilteredEvents(
            data.map((ev: any, i: number) => ({
              ...
              name: ev.displayName,
              description: fallback[i % Math.max(fallback.length, 1)]?.description ?? ev.location,
              price: ev.price,
              date: fallback[i % Math.max(fallback.length, 1)]?.date ?? "TBD",
              capacity: fallback[i % Math.max(fallback.length, 1)]?.capacity ?? 20,
            }))
        );
      } else {
